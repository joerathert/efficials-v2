...
class _SelectCrewScreenState extends State<SelectCrewScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      floatingActionButton: FloatingActionButton(
        ...
        child: const Icon(Icons.filter_list, size: 30, color: AppColors.efficialsYellow),
      ),
  ...
  Widget _buildCrewCard(Crew crew, Map<String, dynamic> args) {
    ...
    return Card(
      color: isSelected ? AppColors.efficialsYellow.withOpacity(0.1) : AppColors.efficialsBlack,
  ...
  Future<void> _loadFilteredCrews() async {
    print('🎯 CREW SELECT: Starting to load filtered crews');
    ...
    try {
      ...
      final crewSizes = <String>[];
      final crewSizeMap = _filterSettings!['crewSizes'] as Map<String, dynamic>?;
      if (crewSizeMap != null) {
        crewSizeMap.forEach((size, isSelected) {
          if (isSelected == true) {
            crewSizes.add(size);
          }
        });
      ...
      print('🎯 CREW SELECT: Applying filters - IHSA: $ihsaCertifications, Levels: $competitionLevels, Sizes: $crewSizes, Distance: $maxDistance');
      final filteredCrews = await _crewRepo.getFilteredCrews(
        ...
        crewSizes: crewSizes.isNotEmpty ? crewSizes : null,
        maxDistanceMiles: maxDistance,
        gameLocation: gameLocation,
      );
      print('🎯 CREW SELECT: getFilteredCrews returned ${filteredCrews.length} crews');
  ...
  void _toggleCrewExpansion(String crewId) {
    setState(() {
